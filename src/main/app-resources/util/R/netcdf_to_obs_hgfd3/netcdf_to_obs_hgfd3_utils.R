...
#
# This version has some updated code for hydrogfd3 in separate functions tagged .hgfd3
#
# Inputs:
#
...
#
# usage:
#   + see example use in /data/proj/Fouh/Global/Arctic-HYPE/Arctic-HYPEv4.0/Model/yakutiahype_42/forcing/scripts/
#
# Last updated 2020-04-13, David Gustafsson, SMHI
# -----------------------------------------------
# Updates (2019-12 until 2020-04-13):
#    + new functions to handle hydrogfd3 data structure on SMHI file server (also dependent on usage)
#    + see example use in /data/proj/Fouh/Global/Arctic-HYPE/Arctic-HYPEv4.0/Model/yakutiahype_42/forcing/scripts/
#                 
#
# Updates (2019-10-08):
...
# -------------------------------------------------------------------------------------------
{
  ...
  {
    # Check if app.sys variable globally set in sourcing code
    if(! exists('app.sys')){
        app.sys = 'local' # 'server' 'tep'
    }
    ...
    if(app.sys == 'tep'){
        fileToSource <- paste(Sys.getenv("_CIOP_APPLICATION_PATH"), "util/R/netcdf_to_obs_hgfd3/netcdf_to_obs_utils_hypetools.R",sep="/")
    }else{
        #fileToSource <- paste(currentDir, "netcdf_to_obs/netcdf_to_obs_utils_hypetools.R",sep="/")
        fileToSource <- paste(currentDir, "netcdf_to_obs_utils_hypetools.R",sep="/")
    }
    if (! file.exists(fileToSource)){
        #rciop.log("INFO", paste0("Aborting netcdf to obs - file missing: ",fileToSource))
        print(paste0("Aborting netcdf to obs - file missing: ",fileToSource)) # replace by cmn.log()
        q(save="no", status = 1)
    }
    source(fileToSource)
  }
...
# -------------------------------------------------------------------------------------------
{
  ...
  {
    # Make spatial layers for the GFD (v1 and v2) gridpoints (point and polygon)
    ...
    # Make spatial layers for the GFD (v3) gridpoints (point and polygon)
    makeGFDv3SpatialLayers <- function(grid.nc=NULL,elev.nc=NULL,dsn=NULL,layer.point=NULL,layer.poly=NULL,xName="lon",yName="lat",zName="elevation"){
      # read netcdf file with the grid dimensions and elevation
      {
        if(!is.null(grid.nc) & !is.null(elev.nc)){
          if(!file.exists(grid.nc) | !file.exists(elev.nc)){
            print("ERROR: grid.nc input is NULL (or file missing) - unable to generate point and polygon layer without input geometry file!")
            print("SOLUTION: >>provide netcdf file with same geometry as the data you want to process")
            print("          >>elevation data is optional - if missing it will be set to 0")
            print("          >>x, y dimension names should be provided on input (xName,yName)")
          ...
          # open nc files with grid dimensions and elevation
          nc.grid = nc_open(grid.nc)
          nc.elev = nc_open(elev.nc)
          
          # check dimension names
          nc.dim.names.elev = names(nc.elev$dim)
          ilon.elev = which(substr(nc.dim.names.elev,1,3)==xName)
          ilat.elev = which(substr(nc.dim.names.elev,1,3)==yName)
          
          nc.dim.names.grid = names(nc.grid$dim)
          ilon.grid = which(substr(nc.dim.names.grid,1,3)==xName)
          ilat.grid = which(substr(nc.dim.names.grid,1,3)==yName)
          
          # lon, lat length
          nc.elev.nlon=nc.elev$dim[[ilon.elev]]$len
          nc.elev.nlat=nc.elev$dim[[ilat.elev]]$len
          nc.grid.nlon=nc.grid$dim[[ilon.grid]]$len
          nc.grid.nlat=nc.grid$dim[[ilat.grid]]$len
          
          # lon and lat values
          nc.elev.lon=round(nc.elev$dim[[ilon.elev]]$vals, 4)
          nc.elev.lat=round(nc.elev$dim[[ilat.elev]]$vals, 4)
          nc.grid.lon=round(nc.grid$dim[[ilon.grid]]$vals, 4)
          nc.grid.lat=round(nc.grid$dim[[ilat.grid]]$vals, 4)
          
          # elevation values
          nc.elev.var.names = names(nc.elev$var)
          ielev = which(substr(nc.elev.var.names,1,nchar(zName))==zName)
          if(length(ielev)>0){
            nc.elev.var.name = nc.elev.var.names[ielev[1]]
          }else{
            nc.elev.var.name = NULL
          }
          if(!is.null(nc.elev.var.name)){
            nc.elev.var = nc.elev$var[[nc.elev.var.name]]
            nc.elev.val = ncvar_get(nc = nc.elev,varid = nc.elev.var,start = c(1,1,1), count=c(nc.elev.nlon,nc.elev.nlat,1))
          }else{
            nc.elev.var = NULL
            nc.elev.val = NULL
          ...
          # grid values
          nc.grid.var.names = names(nc.grid$var)
          igrid = 1
          if(length(ielev)>0){
            nc.grid.var.name = nc.grid.var.names[igrid[1]]
          }else{
            nc.grid.var.name = NULL
          }
          if(!is.null(nc.grid.var.name)){
            nc.grid.var = nc.grid$var[[nc.grid.var.name]]
            nc.grid.val = ncvar_get(nc = nc.grid,varid = nc.grid.var,start = c(1,1,1), count=c(nc.grid.nlon,nc.grid.nlat,1))
          }else{
            nc.grid.var = NULL
            nc.grid.val = NULL
          }
      ...
      # Point layer at GFD grid centre coordinates
      {
        # matrices with data on same form as nc.grid matrix
        gfd.lon   = mat.or.vec(nr = nc.grid.nlon,nc = nc.grid.nlat)
        gfd.lat   = mat.or.vec(nr = nc.grid.nlon,nc = nc.grid.nlat)
        gfd.row   = mat.or.vec(nr = nc.grid.nlon,nc = nc.grid.nlat)
        gfd.col   = mat.or.vec(nr = nc.grid.nlon,nc = nc.grid.nlat)
        gfd.id    = mat.or.vec(nr = nc.grid.nlon,nc = nc.grid.nlat)
        
        # matrices with data on same form as nc.elev matrix
        gfd.lon.e   = mat.or.vec(nr = nc.elev.nlon,nc = nc.elev.nlat)
        gfd.lat.e   = mat.or.vec(nr = nc.elev.nlon,nc = nc.elev.nlat)
        gfd.row.e   = mat.or.vec(nr = nc.elev.nlon,nc = nc.elev.nlat)
        gfd.col.e   = mat.or.vec(nr = nc.elev.nlon,nc = nc.elev.nlat)
        gfd.id.e    = mat.or.vec(nr = nc.elev.nlon,nc = nc.elev.nlat)
        
        if(is.null(nc.elev.val)){
          gfd.elev.e = 0 * gfd.lon.e
          gfd.elev   = 0 * gfd.lon
        }else{
          gfd.elev.e  = nc.elev.val
          gfd.elev    = NA_real_ * gfd.lon
        }
        gfd.id.e[1:(nc.elev.nlon*nc.elev.nlat)]=1:(nc.elev.nlon*nc.elev.nlat)
        gfd.id[1:(nc.grid.nlon*nc.grid.nlat)]=1:(nc.grid.nlon*nc.grid.nlat)
        for(i in 1:nc.elev.nlon){
          gfd.lat.e[i,] =  nc.elev.lat
          gfd.col.e[i,] =  i
        }
        for(i in 1:nc.elev.nlat){
          gfd.lon.e[,i] =  nc.elev.lon
          gfd.row.e[,i] =  i
        }
        for(i in 1:nc.grid.nlon){
          gfd.lat[i,] =  nc.grid.lat
          gfd.col[i,] =  i
        }
        for(i in 1:nc.grid.nlat){
          gfd.lon[,i] =  nc.grid.lon
          gfd.row[,i] =  i
        }
        
        # also make mask for the GFD data
        gfd.mask = nc.grid.val
        gfd.mask[which(!is.na(gfd.mask))]=1
        
        # data frame for the centre-point shapefiles
        gfd.point = data.frame(
          "lon"=gfd.lon[1:(nc.grid.nlon*nc.grid.nlat)],
          "lat"=gfd.lat[1:(nc.grid.nlon*nc.grid.nlat)],
          "row"=gfd.row[1:(nc.grid.nlon*nc.grid.nlat)],
          "col"=gfd.col[1:(nc.grid.nlon*nc.grid.nlat)],
          "id"=gfd.id[1:(nc.grid.nlon*nc.grid.nlat)],
          "elev"=gfd.elev[1:(nc.grid.nlon*nc.grid.nlat)],
          "mask"=gfd.mask[1:(nc.grid.nlon*nc.grid.nlat)],
          "x"=gfd.lon[1:(nc.grid.nlon*nc.grid.nlat)],
          "y"=gfd.lat[1:(nc.grid.nlon*nc.grid.nlat)])
        coordinates(gfd.point)<-c("x","y")
        crs(gfd.point)=CRSlatlon
        
        gfd.point.e = data.frame(
          "lon"=gfd.lon.e[1:(nc.elev.nlon*nc.elev.nlat)],
          "lat"=gfd.lat.e[1:(nc.elev.nlon*nc.elev.nlat)],
          "row"=gfd.row.e[1:(nc.elev.nlon*nc.elev.nlat)],
          "col"=gfd.col.e[1:(nc.elev.nlon*nc.elev.nlat)],
          "id"=gfd.id.e[1:(nc.elev.nlon*nc.elev.nlat)],
          "elev"=gfd.elev.e[1:(nc.elev.nlon*nc.elev.nlat)],
          "x"=gfd.lon.e[1:(nc.elev.nlon*nc.elev.nlat)],
          "y"=gfd.lat.e[1:(nc.elev.nlon*nc.elev.nlat)])
...
#        elev.int = interpp(x = gfd.lon.e,y = gfd.lat.e,z = gfd.elev.e[1:(nc.elev.nlon*nc.elev.nlat)],xo = gfd.lon[iGFD3],yo = gfd.lat[iGFD3])
...
#        gfd.point.out = gfd.point[iGFD3,]
      ...
      # Polygon layer representing grid cells
      {
        ...
        # 1. Determine raster extent and resolution from the incoming netcdf:
        xresolution = (max(nc.grid.lon) - min(nc.grid.lon)) / (nc.grid.nlon-1)
        yresolution = (max(nc.grid.lat) - min(nc.grid.lat)) / (nc.grid.nlat-1)
        
        xmin = min(nc.grid.lon) - 0.5*xresolution
        xmax = max(nc.grid.lon) + 0.5*xresolution
        
        ymin = min(nc.grid.lat) - 0.5*yresolution
        ymax = max(nc.grid.lat) + 0.5*yresolution
        
        # 2. make raster
        nc.grid.raster=raster(nrows=nc.grid.nlat,ncols=nc.grid.nlon,xmn=xmin,xmx=xmax, ymn=ymin,ymx=ymax,vals=NULL)
        crs(nc.grid.raster) <- CRSlatlon
        
        # swap 2nd dimension in order to write correctly to the raster object
        nc.grid.val[,1:nc.grid.nlat]=nc.grid.val[,nc.grid.nlat:1]
        
        # assign values in raster object
        values(nc.grid.raster) = nc.grid.val[1:(nc.grid.nlon*nc.grid.nlat)]
        #plot(nc.grid.raster)
  ...
  {
    # hgfd3 version - Find GFD grid cells overlaying an Area Of Interest
    gridCellsOverAOI.hgfd3 <- function(aoi, grid.poly, CRSproj=NULL,doPlot=F){
      
      # initiate list array for output
      gridCellListArray = array(list(NULL),c(length(aoi),1))
      
      # make sure aoi and gfd.poly have same CRS
      if(as.character(proj4string(aoi))!=as.character(proj4string(grid.poly))){
        aoi=spTransform(aoi,proj4string(grid.poly))
      }
      
      # hgfd3 grid coordinates
      grid.coord=coordinates(grid.poly)
      
      # hgfd3 land mask
      grid.mask = grid.poly@data$mask
      iMask = which(!is.na(grid.mask))
      ...
      for(j in 1:length(aoi)){
        ...
        # GFD grid cells covering the extent, which are also within the GFD land mask
        iExt = which(grid.coord[,1]>aoi.ext[1]-0.5*xresolution & grid.coord[,1]<aoi.ext[2]+0.5*xresolution & 
                       grid.coord[,2]>aoi.ext[3]-0.5*yresolution & grid.coord[,2]<aoi.ext[4]+0.5*yresolution & !is.na(grid.mask))
        
        if(length(iExt)>0){
          # aoi polygon(s) over gfd polygons
          gridOverAoi = testOver(grid.poly[iExt,],aoi[j,])
          
          if(!is.null(gridOverAoi)){
            iOver = iExt[which(!is.na(gridOverAoi[,1]))]
          }else{
            iOver = iExt
          }
          
          # also determine nearest GFD gridpoint to the aoi centre coordinate (in case no land-masked gridcells were found)
          if(is.null(CRSproj)){
          ...
          grid.proj = spTransform(grid.poly[iOver,],CRSprojTemp)
          aoi.xy=coordinates(aoi.proj)
          grid.xy=coordinates(grid.proj)
          xyDiff=((aoi.xy[1]-grid.xy[,1])^2 + (aoi.xy[2]-grid.xy[,2])^2)^0.5
          iNearest=iOver[which(xyDiff==min(xyDiff))[1]]
          
          # data frame with overlaying gridcell information
          gridCellsData = as.data.frame(grid.poly@data[iOver,])
          
          # intersection area, km2
          gridCellsData$intarea=0
          for(i in 1:nrow(gridCellsData)){
            # print(i)
            testArea = IntersectionArea(aoi.proj,grid.proj[i,])
            if(!is.null(testArea)){
              gridCellsData$intarea[i]=testArea
            }else{
              gridCellsData$intarea[i]=-1
            }
          }
          gridCellsData$intarea=gridCellsData$intarea*1E-6
          
          # if the subid only exist in one grid area set its area to 1
          # this is needed for points which has 0 area
          if(length(gridCellsData$intarea)==1){
            gridCellsData$intarea = 1
          }
          
          # handle failed intersection area
          iFail = which(gridCellsData$intarea<0)
          if(length(iFail)>0){
            iOk=which(gridCellsData$intarea>0)
            if(length(iOk)>0){
              gridCellsData$intarea[iFail]=mean(gridCellsData$intarea[iOk])
            }else{
              gridCellsData$intarea=1
            }
          }
          ...
          # weight
          gridCellsData$weight=gridCellsData$intarea/sum(gridCellsData$intarea)
          gridCellListArray[[j]]=list("aoi"=aoi[j,],"cellData"=gridCellsData,"grid.poly"=grid.poly[iOver,],"iOver"=iOver,"iNearest"=iNearest,"nearestData"=gridCellsData[which(iOver==iNearest),],"outsideMask"=F)
          
          # plot
          if(doPlot){
            plot(grid.poly[iOver,],border="red")
            plot(grid.poly[iNearest,],border="green",add=T)
            plot(aoi[j,],border="blue",add=T)
          }
          
        }else{
          # find nearest gridpoint (in lat-lon) within the land mask
          {
            print("outside mask")
            # aoi coordinates
            aoi.xy=coordinates(aoi[j,])
            xyDiff=((aoi.xy[1]-grid.coord[iMask,1])^2 + (aoi.xy[2]-grid.coord[iMask,2])^2)^0.5
            iNearest=iMask[which(xyDiff==min(xyDiff))[1]]
            
            # gridcells data
            gridCellsData = as.data.frame(grid.poly@data[iNearest,])
            gridCellsData$intarea = 1
            gridCellsData$weight  = 1
            gridCellListArray[[j]]=list("aoi"=aoi[j,],"cellData"=gridCellsData,"grid.poly"=grid.poly[iNearest,],"iOver"=iNearest,"iNearest"=iNearest,"nearestData"=gridCellsData,"outsideMask"=T)
...
#                                   skipData=T)
    ...
    gridLinkPreparation.hgfd3<-function(grid.path=NULL,grid.pattern=NULL,grid.elev=NULL,var.name=NULL,
      ...
      # Generate gridLink if missing 
      {
        ...
        if(!file.exists(gridLink.file)|redoGridLink){
          ...
          # d. generate new gfdLink list array
          {
            gridLink = gridCellsOverAOI.hgfd3(
              aoi = subbasin.shp
              ,grid.poly = grid.poly
              ,CRSproj = crsProj
              ,doPlot = F)
    ...
    # Add GFD celldata correction factors (annual and/or monthly) to gridLink list array
    addCorrectionFactorsToGridLink<-function(gridLink,id.gfd=NULL,cgfd.yr=NULL,cgfd.mn=NULL){
      if(!is.null(id.gfd) & (!is.null(cgfd.yr)|!is.null(cgfd.mn))){
        for(i in 1:length(gridLink)){
          # cellData for weighted GFD data
          iCellData = match(gridLink[[i]]$cellData$id.gfd,id.gfd)
          if(!is.null(cgfd.yr)){
            gridLink[[i]]$cellData$cgfd.yr = 1
            gridLink[[i]]$cellData$cgfd.yr[which(!is.na(iCellData))] = cgfd.yr[iCellData[which(!is.na(iCellData))]]
          }
          if(!is.null(cgfd.mn)){
            gridLink[[i]]$cellData$cgfd.m01 = 1
            gridLink[[i]]$cellData$cgfd.m02 = 1
            gridLink[[i]]$cellData$cgfd.m03 = 1
            gridLink[[i]]$cellData$cgfd.m04 = 1
            gridLink[[i]]$cellData$cgfd.m05 = 1
            gridLink[[i]]$cellData$cgfd.m06 = 1
            gridLink[[i]]$cellData$cgfd.m07 = 1
            gridLink[[i]]$cellData$cgfd.m08 = 1
            gridLink[[i]]$cellData$cgfd.m09 = 1
            gridLink[[i]]$cellData$cgfd.m10 = 1
            gridLink[[i]]$cellData$cgfd.m11 = 1
            gridLink[[i]]$cellData$cgfd.m12 = 1
            
            gridLink[[i]]$cellData$cgfd.m01[which(!is.na(iCellData))] = cgfd.mn[iCellData[which(!is.na(iCellData))],1]
            gridLink[[i]]$cellData$cgfd.m02[which(!is.na(iCellData))] = cgfd.mn[iCellData[which(!is.na(iCellData))],2]
            gridLink[[i]]$cellData$cgfd.m03[which(!is.na(iCellData))] = cgfd.mn[iCellData[which(!is.na(iCellData))],3]
            gridLink[[i]]$cellData$cgfd.m04[which(!is.na(iCellData))] = cgfd.mn[iCellData[which(!is.na(iCellData))],4]
            gridLink[[i]]$cellData$cgfd.m05[which(!is.na(iCellData))] = cgfd.mn[iCellData[which(!is.na(iCellData))],5]
            gridLink[[i]]$cellData$cgfd.m06[which(!is.na(iCellData))] = cgfd.mn[iCellData[which(!is.na(iCellData))],6]
            gridLink[[i]]$cellData$cgfd.m07[which(!is.na(iCellData))] = cgfd.mn[iCellData[which(!is.na(iCellData))],7]
            gridLink[[i]]$cellData$cgfd.m08[which(!is.na(iCellData))] = cgfd.mn[iCellData[which(!is.na(iCellData))],8]
            gridLink[[i]]$cellData$cgfd.m09[which(!is.na(iCellData))] = cgfd.mn[iCellData[which(!is.na(iCellData))],9]
            gridLink[[i]]$cellData$cgfd.m10[which(!is.na(iCellData))] = cgfd.mn[iCellData[which(!is.na(iCellData))],10]
            gridLink[[i]]$cellData$cgfd.m11[which(!is.na(iCellData))] = cgfd.mn[iCellData[which(!is.na(iCellData))],11]
            gridLink[[i]]$cellData$cgfd.m12[which(!is.na(iCellData))] = cgfd.mn[iCellData[which(!is.na(iCellData))],12]
          }
        }
        return(gridLink)
      }else{
  ...
  # Function to read GFD (and similar) netCDF files (old, but still used to generate gridLink)
  {
    ...
    getGFDGCMtimeAxis<-function(gfd.file){
      ...
      # POSIX time axis
      {
        ...
        timeZero = substr(nc$dim$time$units,tzPos,nchar(nc$dim$time$units)) # extend to read hours
        ...
        # adjust to zero hours (add 15 seconds for rounding safety)
        timeAxis = as.POSIXct(as.character(timeAxis+15,format="%Y-%m-%d"),tz="GMT")
      }
    ...
    readGFDGCMnetcdf<-function(gfdFile=NULL,           # nc filename
      ...
      if(dataOut){
        # POSIX time axis
        {
          ...
          # timeZero from file
          tzPos = regexpr(pattern = "-",text = nc$dim$time$units)[1]-4
          timeZero = substr(nc$dim$time$units,tzPos,nchar(nc$dim$time$units))
  ...
  # (NEW) functions to read netcdf files from a folder and write HYPE obs files and forckey file to output folder
  {
    ...
    readGridsAndWriteSingleObsFile<-function(grid.files=NULL,
      ...
      for(i in 1:length(iFiles)){
        ...
        # check which time steps should be read
        {
          ...
          # timeAxis read, as POSIX and as months
          if(cgfd.mn){
            timeAxisRead  = timeAxis[iTimeRead]
            monthAxisRead = as.numeric(as.character(timeAxisRead,format="%m"))
          }
        ...
        if(weightedOrNearest==0){
          ...
          for(k in 1:length(obsid)){
            ...
            yrcorr = 1 + 0*(1:nrow(gridLink[[k]]$cellData))
            if(cgfd.yr & length(which(colnames(gridLink[[k]]$cellData)=="cgfd.yr"))>0){
              print("apply cgfd.yr")
              yrcorr=gridLink[[k]]$cellData$cgfd.yr
            }
            
            # monthly correction factors (more complicated with the way we do the weighting here)
            mncorr = 1 + mat.or.vec(nrow(tempData),nrow(gridLink[[k]]$cellData))
            if(cgfd.mn
               & length(which(colnames(gridLink[[k]]$cellData)=="cgfd.m01"))>0
               & length(which(colnames(gridLink[[k]]$cellData)=="cgfd.m02"))>0
               & length(which(colnames(gridLink[[k]]$cellData)=="cgfd.m03"))>0
               & length(which(colnames(gridLink[[k]]$cellData)=="cgfd.m04"))>0
               & length(which(colnames(gridLink[[k]]$cellData)=="cgfd.m05"))>0
               & length(which(colnames(gridLink[[k]]$cellData)=="cgfd.m06"))>0
               & length(which(colnames(gridLink[[k]]$cellData)=="cgfd.m07"))>0
               & length(which(colnames(gridLink[[k]]$cellData)=="cgfd.m08"))>0
               & length(which(colnames(gridLink[[k]]$cellData)=="cgfd.m09"))>0
               & length(which(colnames(gridLink[[k]]$cellData)=="cgfd.m10"))>0
               & length(which(colnames(gridLink[[k]]$cellData)=="cgfd.m11"))>0
               & length(which(colnames(gridLink[[k]]$cellData)=="cgfd.m12"))>0){
              
              print("setup corr-mnt")
              
              imnt=which(monthAxisRead==1)
              if(length(imnt)>0){
                for(jcell in 1:nrow(gridLink[[k]]$cellData)){
                  if(nrow(gridLink[[k]]$cellData)>1){
                    mncorr[imnt,jcell]=gridLink[[k]]$cellData$cgfd.m01[jcell]
                  }else{
                    mncorr[imnt]=gridLink[[k]]$cellData$cgfd.m01
                  }
                }
              }
              imnt=which(monthAxisRead==2)
              if(length(imnt)>0){
                for(jcell in 1:nrow(gridLink[[k]]$cellData)){
                  if(nrow(gridLink[[k]]$cellData)>1){
                    mncorr[imnt,jcell]=gridLink[[k]]$cellData$cgfd.m02[jcell]
                  }else{
                    mncorr[imnt]=gridLink[[k]]$cellData$cgfd.m02
                  }
                }
              }
              imnt=which(monthAxisRead==3)
              if(length(imnt)>0){
                for(jcell in 1:nrow(gridLink[[k]]$cellData)){
                  if(nrow(gridLink[[k]]$cellData)>1){
                    mncorr[imnt,jcell]=gridLink[[k]]$cellData$cgfd.m03[jcell]
                  }else{
                    mncorr[imnt]=gridLink[[k]]$cellData$cgfd.m03
                  }
                }
              }
              imnt=which(monthAxisRead==4)
              if(length(imnt)>0){
                for(jcell in 1:nrow(gridLink[[k]]$cellData)){
                  if(nrow(gridLink[[k]]$cellData)>1){
                    mncorr[imnt,jcell]=gridLink[[k]]$cellData$cgfd.m04[jcell]
                  }else{
                    mncorr[imnt]=gridLink[[k]]$cellData$cgfd.m04
                  }
                }
              }
              imnt=which(monthAxisRead==5)
              if(length(imnt)>0){
                for(jcell in 1:nrow(gridLink[[k]]$cellData)){
                  if(nrow(gridLink[[k]]$cellData)>1){
                    mncorr[imnt,jcell]=gridLink[[k]]$cellData$cgfd.m05[jcell]
                  }else{
                    mncorr[imnt]=gridLink[[k]]$cellData$cgfd.m05
                  }
                }
              }
              imnt=which(monthAxisRead==6)
              if(length(imnt)>0){
                for(jcell in 1:nrow(gridLink[[k]]$cellData)){
                  if(nrow(gridLink[[k]]$cellData)>1){
                    mncorr[imnt,jcell]=gridLink[[k]]$cellData$cgfd.m06[jcell]
                  }else{
                    mncorr[imnt]=gridLink[[k]]$cellData$cgfd.m06
                  }
                }
              }
              imnt=which(monthAxisRead==7)
              if(length(imnt)>0){
                for(jcell in 1:nrow(gridLink[[k]]$cellData)){
                  if(nrow(gridLink[[k]]$cellData)>1){
                    mncorr[imnt,jcell]=gridLink[[k]]$cellData$cgfd.m07[jcell]
                  }else{
                    mncorr[imnt]=gridLink[[k]]$cellData$cgfd.m07
                  }
                }
              }
              imnt=which(monthAxisRead==8)
              if(length(imnt)>0){
                for(jcell in 1:nrow(gridLink[[k]]$cellData)){
                  if(nrow(gridLink[[k]]$cellData)>1){
                    mncorr[imnt,jcell]=gridLink[[k]]$cellData$cgfd.m08[jcell]
                  }else{
                    mncorr[imnt]=gridLink[[k]]$cellData$cgfd.m08
                  }
                }
              }
              imnt=which(monthAxisRead==9)
              if(length(imnt)>0){
                for(jcell in 1:nrow(gridLink[[k]]$cellData)){
                  if(nrow(gridLink[[k]]$cellData)>1){
                    mncorr[imnt,jcell]=gridLink[[k]]$cellData$cgfd.m09[jcell]
                  }else{
                    mncorr[imnt]=gridLink[[k]]$cellData$cgfd.m09
                  }
                }
              }
              imnt=which(monthAxisRead==10)
              if(length(imnt)>0){
                for(jcell in 1:nrow(gridLink[[k]]$cellData)){
                  if(nrow(gridLink[[k]]$cellData)>1){
                    mncorr[imnt,jcell]=gridLink[[k]]$cellData$cgfd.m10[jcell]
                  }else{
                    mncorr[imnt]=gridLink[[k]]$cellData$cgfd.m10
                  }
                }
              }
              imnt=which(monthAxisRead==11)
              if(length(imnt)>0){
                for(jcell in 1:nrow(gridLink[[k]]$cellData)){
                  if(nrow(gridLink[[k]]$cellData)>1){
                    mncorr[imnt,jcell]=gridLink[[k]]$cellData$cgfd.m11[jcell]
                  }else{
                    mncorr[imnt]=gridLink[[k]]$cellData$cgfd.m11
                  }
                }
              }
              imnt=which(monthAxisRead==12)
              if(length(imnt)>0){
                for(jcell in 1:nrow(gridLink[[k]]$cellData)){
                  if(nrow(gridLink[[k]]$cellData)>1){
                    mncorr[imnt,jcell]=gridLink[[k]]$cellData$cgfd.m12[jcell]
                  }else{
                    mncorr[imnt]=gridLink[[k]]$cellData$cgfd.m12
                  }
                }
              }
            }
            ...
            # weighted sum
            if(nrow(gridLink[[k]]$cellData)>1){
              sumData = tempData[,iData[1]]*iWeights[1]*yrcorr[1]*mncorr[,1]
            }else{
              sumData = tempData[,iData[1]]*iWeights[1]*yrcorr[1]*mncorr
            }
            if(length(iData)>1){
              for(l in 2:length(iData)){
                sumData = sumData + tempData[,iData[l]]*iWeights[l]*yrcorr[l]*mncorr[,l]
              }
            }
            ...
            if(length(iMissing)>0){
              for(m in 1:length(iMissing)){
                kk=iMissing[m]
                if(nrow(gridLink[[k]]$cellData)>1){
                  inData = tempData[kk,iData] * mncorr[kk,]
                }else{
                  inData = tempData[kk,iData] * mncorr[kk]
                }
                isData = which(!is.na(inData))
                if(length(isData)>0){
                  sumData[kk] = sum(inData[isData]*iWeights[isData]*yrcorr[isData])/sum(iWeights[isData])
                }
              }
            }
            # update output data
      ...
      # return sucess or last written date if requested
      if(returnEndDate){
        return(obsData[nrow(obsData),1])
      }else{
        return(0)
      }
    }
    ...
    readGridsAndWriteObs.hgfd3<-function(grid.path,grid.pattern,var.name,obs.type,obs.scale=1,obs.offset=0,obs.digits=1,
                                   ...
                                   model.subset=NULL,returnEndDate=F){
      print("NETCDF_TO_OBS:  readGridsAndWriteObs hgfd3 version...")
      ...
      print("... reading gridLink.Rdata")
      {
        ...
        # adjust celldata
        for(i in 1:length(gridLink)){
          gridLink[[i]]$cellData$cell.nc = gridLink[[i]]$cellData$id
        }
      }
      ...
      print("... START reading/writing NETCDF to OBS...")
      {
        ...
        yrs1 = as.numeric(as.character(time.start,format="%Y"))
        yrs2 = as.numeric(as.character(time.end,format="%Y"))
        
        # loop over grid.pattern (obs.type)
        gridFiles = NULL # Tobias
        for(i in 1:length(grid.pattern)){
          
          # list files
          fileIni=0
          for(yrs in yrs1:yrs2){
            for(mnts in c("01","02","03","04","05","06","07","08","09","10","11","12")){
              nextDir = paste(grid.path.vector[i],"/",as.character(yrs),"/",mnts,sep="")
              if(dir.exists(nextDir)){
                newFiles = dir(path = nextDir,pattern = grid.pattern[i])
                if(fileIni==0){
                  gridFiles = paste(nextDir,"/",newFiles,sep="")
                  fileIni=1
                }else{
                  gridFiles = c(gridFiles,paste(nextDir,"/",newFiles,sep=""))
                }
              }
            }
          }
          ...
          if(length(gridFiles)>0){
            ...
            if(returnEndDate){
              if(class(isObsData)[1]=="POSIXct"){
                print(paste("SUCCESS writing obsfile: ",paste(output.path,"/",obs.type[i],".txt",sep=""),sep=""))
                print(paste("  last date in file = ",as.character(isObsData),sep=""))
                lastDateWritten = isObsData
              }else{
    ...
    readGridsAndWriteObs<-function(grid.path,grid.pattern,var.name,obs.type,obs.scale=1,obs.offset=0,obs.digits=1,
                                   gridLink.path=NULL,output.path=".",overwrite=T,doForcKey=T,elev.digits=1,
                                   time.start=NULL,time.end=NULL,weightedOrNearest=0,saveCopyOfExistingObsFiles=T,
                                   model.subset=NULL,cgfd.yr=NULL,cgfd.mn=NULL){
      ...
      print("... START reading/writing NETCDF to OBS...")
      {
        ...
        # make sure obs.scale,obs.offset, and obs.digits have same length as grid pattern
        {
          ...
          if(!is.null(cgfd.yr)){
            if(length(cgfd.yr)<length(grid.pattern)){
              cgfd.yr2 = cgfd.yr[1]
              for(j in 2:length(grid.pattern)){
                cgfd.yr2 = c(cgfd.yr2,cgfd.yr[1])
              }
              cgfd.yr = cgfd.yr2
            }
          }else{
          ...
          if(!is.null(cgfd.mn)){
            if(length(cgfd.mn)<length(grid.pattern)){
              cgfd.mn2 = cgfd.mn[1]
              for(j in 2:length(grid.pattern)){
                cgfd.mn2 = c(cgfd.mn2,cgfd.mn[1])
              }
              cgfd.mn = cgfd.mn2
            }
          }else{
  ...
  # NEW functions dedicated to hgfd3 Hindcast and Forecast data
  {
    readGridsAndWriteObs.hgfd3.forecast<-function(grid.path,grid.pattern,var.name,obs.type,obs.scale=1,obs.offset=0,obs.digits=1,
                                         gridLink.path=NULL,output.path=".",overwrite=T,doForcKey=T,elev.digits=1,
                                         time.start=NULL,time.end=NULL,weightedOrNearest=0,saveCopyOfExistingObsFiles=T,
                                         model.subset=NULL){
      print("NETCDF_TO_OBS:  readGridsAndWriteObs hgfd3 forecast version...")
      ...
      print("... START reading/writing NETCDF to OBS...")
      {
        ...
        yr = as.character(time.start,format="%Y")
        mo = as.character(time.start,format="%m")
        da = as.character(time.start,format="%d")
        
        # full path to forecast files based on file patterns and issue date (time.start)
        forecast.files = paste(grid.path,"/",yr,"/",mo,"/",grid.pattern,sep="")
        for(i in 1:length(grid.pattern)){
          substr(forecast.files[i],nchar(forecast.files[i])-10,nchar(forecast.files[i])-7) = yr
          substr(forecast.files[i],nchar(forecast.files[i])-6,nchar(forecast.files[i])-5) = mo
          substr(forecast.files[i],nchar(forecast.files[i])-4,nchar(forecast.files[i])-3) = da
        }
        
        # loop over grid.pattern (obs.type)
        for(i in 1:length(grid.pattern)){
          if(file.exists(forecast.files[i])){
            gridFiles = forecast.files[i]    
          }else{
            gridFiles = NULL
          }
          ...
          if(length(gridFiles)>0){
            ...
            print(paste("missing forecast file: ",forecast.files[i],sep=""))
            return(-1)
          }
        }
      }
